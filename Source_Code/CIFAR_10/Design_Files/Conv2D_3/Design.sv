module conv2d_8_8_64_batchnorm_relu6_1x_128ch (
    ...
    // NOTE (fix): the total weight count is IN_CH*OUT_CH*KERNEL*KERNEL =
    // 64*128*3*3 = 73728, split across 4 ROM chunks of 18432 entries each.
    // get_weight() below already assumed weight_rom2/3/4 existed, but only
    // weight_rom1 was ever declared -- that made this module fail to
    // elaborate. The three missing ROMs are added here, sized to match.
    localparam int ROM_CHUNK = 18432;
    reg signed [7:0] weight_rom1 [0:ROM_CHUNK-1];
    reg signed [7:0] weight_rom2 [0:ROM_CHUNK-1];
    reg signed [7:0] weight_rom3 [0:ROM_CHUNK-1];
    reg signed [7:0] weight_rom4 [0:ROM_CHUNK-1];
    reg signed [7:0] bias_mem   [0:OUT_CH-1];
    reg signed [7:0] scale      [0:OUT_CH-1];
    reg signed [7:0] shift      [0:OUT_CH-1];

    // NOTE: these were declared but never initialized in the original
    // source, leaving them X in simulation. See CHANGELOG.md.
    integer init_i;
    initial begin
        for (init_i = 0; init_i < ROM_CHUNK; init_i = init_i + 1) begin
            weight_rom1[init_i] = 8'sd0;
            weight_rom2[init_i] = 8'sd0;
            weight_rom3[init_i] = 8'sd0;
            weight_rom4[init_i] = 8'sd0;
        end
        for (init_i = 0; init_i < OUT_CH; init_i = init_i + 1) begin
            bias_mem[init_i] = 8'sd0;
            scale[init_i]    = 8'sd1;
            shift[init_i]    = 8'sd0;
        end
        $readmemh("cifar10_conv3_weights_block0.hex", weight_rom1);
        $readmemh("cifar10_conv3_weights_block1.hex", weight_rom2);
        $readmemh("cifar10_conv3_weights_block2.hex", weight_rom3);
        $readmemh("cifar10_conv3_weights_block3.hex", weight_rom4);
        $readmemh("cifar10_conv3_bias.hex",    bias_mem);
        $readmemh("cifar10_conv3_scale.hex",   scale);
        $readmemh("cifar10_conv3_shift.hex",   shift);
    end
