module dense_layer_1x_2048_to_128_bn_relu6 (
    ...
    // source, leaving them X in simulation. See CHANGELOG.md. (The dense
    // weights themselves live in dense_weights_rom_block0, not here.)
    integer init_i;
    initial begin
        for (init_i = 0; init_i < OUT_DIM; init_i = init_i + 1) begin
            biases[init_i] = 8'sd0;
            scale[init_i]  = 8'sd1;
            shift[init_i]  = 8'sd0;
        end
        $readmemh("cifar10_dense1_bias.hex",  biases);
        $readmemh("cifar10_dense1_scale.hex", scale);
        $readmemh("cifar10_dense1_shift.hex", shift);
    end
    ...
    cifar10_dense_weights_rom_block0 rom(.addr(weights_addr), .data(weights_data));
