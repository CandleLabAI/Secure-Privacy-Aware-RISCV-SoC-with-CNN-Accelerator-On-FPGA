module conv2d_16_16_32_batchnorm_relu6_1x_64ch(
    ...
    integer init_i;
    initial begin
        for (init_i = 0; init_i < IN_CH*OUT_CH*KERNEL*KERNEL; init_i = init_i + 1) weight_mem[init_i] = 8'sd0;
        for (init_i = 0; init_i < OUT_CH; init_i = init_i + 1) begin
            bias_mem[init_i] = 8'sd0;
            scale[init_i]    = 8'sd1;
            shift[init_i]    = 8'sd0;
        end
        $readmemh("cifar10_conv2_weights.hex", weight_mem);
        $readmemh("cifar10_conv2_bias.hex",    bias_mem);
        $readmemh("cifar10_conv2_scale.hex",   scale);
        $readmemh("cifar10_conv2_shift.hex",   shift);
    end
