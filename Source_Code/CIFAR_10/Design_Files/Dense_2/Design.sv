...
module dense_layer_2x_128_to_10 (
    ...
    integer init_i;
    initial begin
        for (init_i = 0; init_i < IN_DIM*OUT_DIM; init_i = init_i + 1) weights[init_i] = 8'sd0;
        for (init_i = 0; init_i < OUT_DIM;        init_i = init_i + 1) biases[init_i]  = 8'sd0;
        $readmemh("cifar10_dense2_weights.hex", weights);
        $readmemh("cifar10_dense2_bias.hex",    biases);
    end
