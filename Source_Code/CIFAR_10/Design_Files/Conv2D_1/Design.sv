module conv2d_32_32_3_batchnorm_relu6_1x_32ch(
    ...
    integer init_i;
    initial begin
        for (init_i = 0; init_i < IM_W*IM_H*NUM_CH; init_i = init_i + 1) image_mem[init_i]  = 8'sd0;
        for (init_i = 0; init_i < WEIGHT_NUM;       init_i = init_i + 1) weight_mem[init_i] = 8'sd0;
        for (init_i = 0; init_i < NUM_FILT;         init_i = init_i + 1) begin
            bias_mem[init_i] = 8'sd0;
            scale[init_i]    = 8'sd1;
            shift[init_i]    = 8'sd0;
        end
        $readmemh("cifar10_conv1_image.hex",   image_mem);
        $readmemh("cifar10_conv1_weights.hex", weight_mem);
        $readmemh("cifar10_conv1_bias.hex",    bias_mem);
        $readmemh("cifar10_conv1_scale.hex",   scale);
        $readmemh("cifar10_conv1_shift.hex",   shift);
    end
    ...
    genvar pe;
    generate
        // NOTE (fix): loop bound was 64, but a_in/b_in/pe_result only hold
        // 27 entries (NUM_CH*3*3 = 3*9 = 27, matching the idx accumulation
        // loop below). The extra 37 iterations indexed out of bounds,
        // which is undefined in simulation and would not synthesize
        // consistently. Bounded to 27 to match actual usage.
        for (pe = 0; pe < 27; pe = pe + 1) begin : PE_ARRAY
            wire signed [15:0] pe_out_w;
            processing_element pe_inst (
                .a(a_in[pe]),
                .b(b_in[pe]),
                .result(pe_out_w)
            );
            assign pe_result[pe] = pe_out_w;
        end
    endgenerate
