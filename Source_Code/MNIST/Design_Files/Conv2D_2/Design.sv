module conv2d_13_13_32_to_11_11_64(
    ...
    integer init_i;
    initial begin
        for (init_i = 0; init_i < OC*IC*K*K; init_i = init_i + 1) weights[init_i] = 8'sd0;
        for (init_i = 0; init_i < OC;        init_i = init_i + 1) biases[init_i]  = 8'sd0;
        $readmemh("mnist_conv2_weights.hex", weights);
        $readmemh("mnist_conv2_bias.hex",    biases);
    end
    reg        maxpool_start;
    wire       maxpool_done;
