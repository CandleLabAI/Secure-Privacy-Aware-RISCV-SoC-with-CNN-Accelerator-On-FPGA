module conv2d_3x3_engine_32ch_pes (
    ...
    // source, leaving them X in simulation. Loaded here from per-module
    // hex files (placeholder zero-filled data ships with this repo -- see
    // CHANGELOG.md for how to regenerate real trained values).
    integer init_i;
    initial begin
        for (init_i = 0; init_i < 784; init_i = init_i + 1) image_mem[init_i] = 8'sd0;
        for (init_i = 0; init_i < 288; init_i = init_i + 1) weight_mem[init_i] = 8'sd0;
        for (init_i = 0; init_i < 32;  init_i = init_i + 1) bias_mem[init_i]   = 8'sd0;
        $readmemh("mnist_conv1_image.hex",  image_mem);
        $readmemh("mnist_conv1_weights.hex", weight_mem);
        $readmemh("mnist_conv1_bias.hex",   bias_mem);
    end
