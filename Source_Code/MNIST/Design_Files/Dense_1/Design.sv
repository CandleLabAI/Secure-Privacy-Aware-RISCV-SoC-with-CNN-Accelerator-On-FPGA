module dense_layer_1600_to_128(
    ...
    // NOTE (fix): clk was never connected to any of these 8 ROM instances
    // in the original source -- their registered outputs would never
    // update in simulation or hardware. Wired to this module's clk.
    dense_weights_rom_block0 rom0(.clk(clk), .addr(weights_addr), .data(weights_data[0]));
    dense_weights_rom_block1 rom1(.clk(clk), .addr(weights_addr), .data(weights_data[1]));
    dense_weights_rom_block2 rom2(.clk(clk), .addr(weights_addr), .data(weights_data[2]));
    dense_weights_rom_block3 rom3(.clk(clk), .addr(weights_addr), .data(weights_data[3]));
    dense_weights_rom_block4 rom4(.clk(clk), .addr(weights_addr), .data(weights_data[4]));
    dense_weights_rom_block5 rom5(.clk(clk), .addr(weights_addr), .data(weights_data[5]));
    dense_weights_rom_block6 rom6(.clk(clk), .addr(weights_addr), .data(weights_data[6]));
    dense_weights_rom_block7 rom7(.clk(clk), .addr(weights_addr), .data(weights_data[7]));

    initial begin
        $readmemh("mnist_dense1_bias.hex", biases);
    end
