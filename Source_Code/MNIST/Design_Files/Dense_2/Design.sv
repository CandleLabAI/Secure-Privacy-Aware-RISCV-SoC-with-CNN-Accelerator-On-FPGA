module dense_layer_128_to_10(
    ...
    integer init_o, init_i;
    initial begin
        for (init_o = 0; init_o < OUT_DIM; init_o = init_o + 1) begin
            biases[init_o] = 8'sd0;
            for (init_i = 0; init_i < IN_DIM; init_i = init_i + 1)
                weights[init_o][init_i] = 8'sd0;
        end
        $readmemh("mnist_dense2_weights.hex", weights);
        $readmemh("mnist_dense2_bias.hex",    biases);
    end
    ...
    dense_128_bram_wrapper_2 dense_last_10_bram (
